...
const trendFormSchema = z.object({
  platform: z.string({ required_error: "Please select a platform." }).min(1),
  niche: z.string({ required_error: "Please select a niche." }).min(1),
  ...
  async function onTrendSubmit(values: TrendFormValues) {
    ...
    try {
      ...
      toast({
        ...
        description:
          "An unexpected error occurred. The selected AI model may be overloaded. Please try again later.",
      });
    } finally {
  ...
  return (
    <div className="container mx-auto px-4 py-8 md:py-12">
      <header className="text-center mb-12">
        <h1 className="font-headline text-4xl md:text-6xl font-bold tracking-tighter">Find Your Next Viral Hit</h1>
        <p className="mt-4 text-md md:text-lg text-muted-foreground max-w-2xl mx-auto">
          Our AI forecasts emerging social media trends 24-72 hours before they go mainstream. Find your next viral video
          idea now.
        </p>
      </header>
              ...
              <form onSubmit={trendForm.handleSubmit(onTrendSubmit)} className="space-y-8">
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                  <Button
                    ...
                    className="w-full sm:w-auto font-bold text-lg py-6 px-8 flex-grow"
                    size="lg"
                  >
      ...
      <section className="mt-12">
        {isLoadingTrends && (
          <div className="space-y-4">
            <h2 className="font-headline text-3xl font-bold mb-6 text-center animate-pulse">
              Scanning the Socialverse...
            </h2>
          ...
          <div>
            ...
            <div className="space-y-4 max-w-4xl mx-auto">
              {trends.map((trend, index) => {
                const niche =
                  trendForm.getValues("niche") === "Other"
                    ? trendForm.getValues("otherNiche")
                    : trendForm.getValues("niche");
                const microNiche =
                  trendForm.getValues("microNiche") === "Other"
                    ? trendForm.getValues("otherMicroNiche")
                    : trendForm.getValues("microNiche");
                const nicheValue = microNiche ? `${niche} (${microNiche})` : niche;

                return <TrendCard key={index} trend={trend} context={{ platform: trendForm.getValues("platform"), niche: nicheValue || "", region: trendForm.getValues("region") }} />;
              })}
            </div>
          </div>
        )}
