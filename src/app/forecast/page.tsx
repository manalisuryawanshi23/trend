...
export default function TrendingPage() {
  ...
  const { uniqueNiches, uniquePlatforms, filteredTrends } = useMemo(() => {
    if (!trendsData)
      return { uniqueNiches: [], uniquePlatforms: [], filteredTrends: [] };

    const trends = trendsData.trends;
    const uniqueNiches = [
      "all",
      ...Array.from(new Set(trends.map((t) => t.niche))),
    ];
    const uniquePlatforms = [
      "all",
      ...Array.from(new Set(trends.map((t) => t.platform))),
    ];

    const filtered = trends.filter((trend) => {
      const nicheMatch =
        selectedNiche === "all" || trend.niche === selectedNiche;
      const platformMatch =
    ...
    const sortedAndGrouped = Object.entries(grouped).sort(
    ...
    <div className="container mx-auto px-4 py-8 md:py-12">
      <header className="text-center mb-12">
        ...
        <p className="mt-4 text-md md:text-lg text-muted-foreground max-w-2xl mx-auto">
          A real-time snapshot of the top emerging trends across the internet,
          powered by AI.
        </p>
      </header>
      ...
      <Card className="p-4 sm:p-6 mb-8 max-w-4xl mx-auto shadow-lg border-2 border-primary/10">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 items-center">
          <h3 className="font-headline text-lg font-semibold sm:col-span-1 flex items-center gap-2">
      ...
      <div className="space-y-12">
        {filteredTrends.length > 0 ? (
          filteredTrends.map(([niche, nicheTrends]) => (
            <section key={niche}>
              <h2 className="font-headline text-2xl font-bold mb-6 inline-block rounded-md bg-muted px-4 py-2">
                {niche}
              </h2>
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                  ...
                  <TrendDetailDialog key={trend.trendName} trend={trend}>
                    <Card className="flex flex-col overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 border hover:border-primary/50 h-full cursor-pointer">
                      <CardHeader>
                        ...
                        <h3 className="font-headline text-xl font-bold pt-2">
                          {trend.trendName}
                        </h3>
                      </CardHeader>
                      <CardContent className="flex-grow">
